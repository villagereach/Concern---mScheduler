...
public class CalendarClient
{
    ...
    EventEntry executeInsert(EventEntry entry, CalendarUrl url) throws IOException
    {
        AtomContent content = new AtomContent();
        content.namespaceDictionary = DICTIONARY;
        content.entry = entry;
    ...
    public EventEntry executeUpdateEvent(EventEntry updated) throws IOException
    {
        AtomContent content = new AtomContent();
        content.namespaceDictionary = DICTIONARY;
        content.entry = updated;

        HttpRequest request = requestFactory.buildPutRequest(new GenericUrl(updated.getEditLink()), content);
        request.headers.ifMatch = "*";
        return request.execute().parseAs(updated.getClass());
    }
    ...
    public EventEntry execInsertEventEntry(CalendarUrl calendarUrl, EventEntry e) throws IOException
    {
        return executeInsert(e, calendarUrl);
    }
}
